package com.joker.mmsbackenduserservice.service.Impl;
...
import com.joker.mmsbackendmodel.dto.user.*;
...
import com.joker.mmsbackendmodel.vo.LoginUserVO;
...
import com.joker.mmsbackenduserservice.mapper.UserMapper;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword) {
        ...
        if (userPassword.length() < 8 || checkPassword.length() < 8) {
            throw new BusinessException(ErrorCode.REQUEST_ERROR, "用户密码过短");
        }
        // 密码和校验密码相同
    ...
    public LoginUserVO userLogin(String userAccount, String userPassword, HttpServletRequest request) {
        ...
        if (userAccount.length() < 4) {
            throw new BusinessException(ErrorCode.REQUEST_ERROR, "账号错误");
        }
        if (userPassword.length() < 8) {
            throw new BusinessException(ErrorCode.REQUEST_ERROR, "密码错误");
        }
        // 2. 加密
        String encryptPassword = DigestUtils.md5DigestAsHex((SALT + userPassword).getBytes());
        // 查询用户是否存在
        ...
        if (user == null) {
            ...
            throw new BusinessException(ErrorCode.REQUEST_ERROR, "用户不存在或密码错误");
        }
        // 3. 记录用户的登录态
